...
class OMEXMLReader(object):
    ...
    tags = {
        'Image': ns + "Image", 'Pixels': ns + "Pixels",
        'BinData': nsb + "BinData", 'Channel': ns + "Channel",
    ...
    def read_meta(self):
        ...
        # based on whether we are opeing a OME-XML or
        #OME-TIFF file the tags contained in the XML are different
        ...
        if self.bintagname == "TiffData":
            ...
            #self.et = xmle.fromstring(\
            #    self.pil_image.tag[image_description_tag])
        else:
            try:
                ...
                with open(self.filename, 'r') as f:
                    ...
                    for i, line in enumerate(lines):
                        change = True in [
                            bad_char in line for bad_char in bad.keys()]
                        if change:
    ...
    def _make_tags(self):
        names = ["Image",
                 "Image/Pixels", "BinData", "Image/AcquisitionDate",
                 "Image/Description",
                 "StructuredAnnotations/XMLAnnotation/Value/OriginalMetadata",
                 "MDKey", 'MDValue', "Pixels", "Channel", "AcquisitionDate",
                 'TiffData', "ROI/Union/Shape/Line"]
        for name in names:
            self.fulltags[name] = self._make_tag(name)

    def _get_image_attributes(self):
        """
        Reads image information from the ome file. Square images are assumed
        to be the reference image (recorded by the microscope before a
        linescan). Linescan image is used for the general image
        attributes as those are the ones relevant to analysis.
        """

        self.logger.info(
            "Reading image attributes for image {}".format(self.filename))
        ...
        for im_n, image_element in enumerate(image_elements):
            ...
            image_stuff = {
                ...
                "Attributes": image_element.attrib,
                "BinDatas": bindata_dict, "ImageData": None}
            for pi_n, pixel in enumerate(pixels):
                # There is only one pixel element per Image
                #(although the schema allows more)
                # TODO In the next OME release Pixel data will be moved up to
                # Image
            ...
            # we can do this because we know only one Pixel element exists per
            # Image
            ...
            message = "Image {}: {} x {} pixels ({} total), {} {}, {} {}".\
                format(self.active_image_number, self.image_width,
                       self.image_height, self.pixels, self.channels,
                       pluralizer.plural("channel", self.channels),
                       self.frames, pluralizer.plural("frame", self.frames))
            if self.image_step_x and self.image_step_y:
                message += ", dx:dy = {} : {}".format(self.image_step_x,
                                                      self.image_step_y)
        ...
        try:
            self.description = self.et.find(
                self.fulltags["Image/Description"]).text
        ...
        #        roi = {"x2":float(roi_attrib["X2"]),
        #               "x1":float(roi_attrib["X1"]),
        #               "y2":float(roi_attrib["Y2"]),
        #               "y1":float(roi_attrib["Y1"])}
    ...
    def _get_image_data(self, image_type):
        self.logger.info(
            "Reading data for image {}".format(self.active_image_number))
        tiffdata_elements = self.images[image_type]["BinDatas"]
        data_array = numpy.zeros(
            shape=(self.channels, self.frames,
                   self.image_height, self.image_width),
            dtype=self.data_type)
        self.images[image_type]["ImageData"] = data_array
        for tiffdata_element_key in tiffdata_elements:
            # since ome does not group bindatas in channels we have to
            # guess which bindata elements are in which channel.
            # assuming that all frames from one channel are grouped
            # together so we have self.frames frames in each channel.
            ...
            # Need to read image dimension from PixelAttribute as they are
            # different for different image types
            image_width = int(
                self.images[image_type]["PixelAttributes"]["SizeX"])
            image_height = int(
                self.images[image_type]["PixelAttributes"]["SizeY"])
        ...
        self.logger.info("\nRead %i channels\n%i frames in each \
                         ...
                         entire array\n" %
                         (self.channels, self.frames, self.image_width,
                          self.image_height, data_array.nbytes / 1024 ** 2))
    ...
    def _get_extra_info(self):
        ...
        for element in self.annotation_elements:
            ...
            if self.image_name in keyin:
                ...
                key = keyin
            self.raw_annotation[key] = element.find(
                self.fulltags["MDValue"]).text
